  ...
  if(Serial.available()){
    ...
    switch(CMD){
      ...
      case '2':
        ...
        Agv_accel(2,max_time_2ms,scale_2ms,100);
        Agv_constant(2,0);
        Agv_decel(2,max_time_2ms,scale_2ms,100);
        break;
      case '1':
        Serial.println("---------------11111----------------");
        Agv_accel(1,max_time_1ms,scale_1ms,50);
        Agv_constant(1,0);
        Agv_decel(1,max_time_1ms,scale_1ms,50);
        break;
      case '4':
        Serial.println("--------------------BBBB-----------");
        Agv_accel(4,max_time_1ms,scale_1ms,50);
        delay(Back_time);
        Agv_decel(4,max_time_1ms,scale_1ms,50);
        break;        
    }
  }
}
...
//AGV 가속함수
void Agv_accel(double vel_type,double Max_time,double scale,int number){
...
//AGV 감속함수
void Agv_decel(double vel_type,double Max_time,double scale,int number){
  while (sec >0){
    cycle_start_time = millis();
    double Vel=VEL_TYPE(vel_type,sec);//속도공식
    double RPM=Vel*60/(3.14*0.058);//모터에 부여할 RPM
